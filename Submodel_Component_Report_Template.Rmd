...
create_individual_map <- function(data, layer_name, score_value, map_title, wea_data = NULL) {
  ...
  if (!is.null(wea_data) && nrow(wea_data) > 0) {
    ...
    map <- map %>%
      ...
      addLayersControl(
        overlayGroups = c("Layer Data", "WEA Boundary"),
...
create_combined_map <- function(combined_data, title, method = "geometric_mean", wea_data = NULL) {
...
if (!is.null(params$selected_methods) && "geometric_mean" %in% params$selected_methods && 
  ...
  combined_data <- params$combined_data_list[["geometric_mean"]]
  if(!is.null(combined_data)) {
    create_combined_map(combined_data, paste(params$combined_map_title, "- Geometric Mean"), "geometric_mean", wea_data = params$wea_data)
  }
}
```
...
if (!is.null(params$selected_methods) && "lowest" %in% params$selected_methods && 
  ...
  combined_data <- params$combined_data_list[["lowest"]]
  if(!is.null(combined_data)) {
    create_combined_map(combined_data, paste(params$combined_map_title, "- Lowest Score"), "lowest", wea_data = params$wea_data)
  }
}
```
...
if (!is.null(params$selected_methods) && "product" %in% params$selected_methods && 
  ...
  combined_data <- params$combined_data_list[["product"]]
  if(!is.null(combined_data)) {
    create_combined_map(combined_data, paste(params$combined_map_title, "- Product"), "product", wea_data = params$wea_data)
  }
}
```
