  ...
  if (
    ...
    map <- map %>%
      addPolygons(
        ...
        fillOpacity = 0.7,
        popup = ~ paste(
          "Z-Membership Cell Score:",
          round(Score.Z_Membership, 3)
        ),
        group = "Data Layer"
      ) %>%
      addLegend(
    ...
    !is.null(config$score) &&
      config$score == "Ranked Importance" &&
      !is.null(config$color_palette)
  ) {
    ...
    if (!is.null(score_column) && score_column %in% names(config$data)) {
      map <- map %>%
        addPolygons(
          ...
          fillOpacity = 0.7,
          popup = ~ paste(
            "Ranked Importance Cell Score:",
            round(get(score_column), 3)
          ),
          group = "Data Layer"
        ) %>%
        addLegend(
    ...
    map <- map %>%
      addPolygons(
        ...
        fillColor = ~ {
          ...
          score_cols <- names(config$data)[grep(
            "^Score\\.",
            names(config$data)
          )]
          if (length(score_cols) > 0) {
            # --- FIXED LEAFLET EXTRACTION ---
            actual_score <- config$data[[score_cols[1]]]

            target_num <- suppressWarnings(as.numeric(config$score))
            ifelse(
              !is.na(target_num) &
                !is.na(actual_score) &
                abs(as.numeric(actual_score) - target_num) < 1e-6,
              config$color,
              "#CCCCCC"
            )
          } else {
      ...
      addLegend(
        ...
        labels = c(
          paste(
            "Selected Score for Offshore Wind Energy Suitability:",
            config$score
          ),
          "Unselected (1)"
        ),
