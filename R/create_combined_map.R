create_combined_map <- function(
  ...
  if (is.null(combined_data) || nrow(combined_data) == 0) {
    ...
    map <- map %>%
      addControl(
        paste("Score column", score_column, "not found"),
        position = "topright"
      )
  } else {
    # Get the range of score values - preserve precision for small values
    score_values <- combined_data[[score_column]][
      !is.na(combined_data[[score_column]])
    ]
    ...
    if (length(score_values) == 0) {
      ...
      combined_data <- combined_data[!is.na(combined_data[[score_column]]), ]
      ...
      tryCatch(
        {
          combined_data$popup_display <- paste(
            popup_prefix,
            ifelse(
              combined_data[[score_column]] < 0.01,
              format(
                combined_data[[score_column]],
                scientific = FALSE,
                digits = 3
              ),
        ...
        tryCatch(
          {
            map <- map %>%
              ...
              addLegend(
                ...
                colors = single_color,
                labels = paste(
                  "Offshore Wind Energy Suitability Score:",
                  ifelse(
                    min_val < 0.01,
        ...
        tryCatch(
          {
            ...
            score_column_data <- tryCatch(
              ...
              error = function(e) {
                cat(
                  "Error converting score column to numeric:",
                  e$message,
                  "\n"
                )
            ...
            if (all(is.na(score_column_data))) {
              ...
              clean_values <- score_column_data[
                !is.na(score_column_data) & is.finite(score_column_data)
              ]
