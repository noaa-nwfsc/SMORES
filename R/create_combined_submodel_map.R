create_combined_submodel_map <- function(
  component_data_list,
  base_grid = grid_test,
  ...
  tryCatch(
    {
      ...
      for (component_name in names(component_data_list)) {
        ...
        score_cols <- names(component_data)[
          names(component_data) %in%
            c("Geo_mean", "Lowest_value", "Product_value")
        ]
        ...
        if (length(score_cols) > 0) {
          ...
          id_col <- grep("^CellID", names(component_data), value = TRUE)[1]

          # Extract data and join dynamically
          temp_data <- component_data %>%
            st_drop_geometry() %>%
            select(all_of(c(id_col, score_col))) %>%
            rename(!!new_col_name := !!score_col)

          combined_data <- left_join(combined_data, temp_data, by = id_col)
        }
      }

      # Calculate geometric mean
      combined_data <- calculate_submodel_geometric_mean(
        combined_data,
      ...
      if (nrow(map_data) == 0) {
        ...
        map <- map %>%
          addControl(
            "No valid data for combined submodel",
            position = "topright"
          )
      } else {
        ...
        score_values <- score_values[
          !is.na(score_values) & is.finite(score_values)
        ]
        ...
        if (length(score_values) == 0) {
          return(list(
            ...
            map = leaflet() %>%
              addProviderTiles("Esri.OceanBasemap") %>%
              addControl(
                "No valid score values for mapping",
                position = "topright"
              )
          ))
        }
        ...
        map_data$popup_display <- paste0(
          "Combined Submodel Offshore Wind Energy Suitability Score:",
          ifelse(
            map_data$Geo_mean < 0.01,
          ...
          map <- tryCatch(
            ...
            error = function(e) {
              ...
              map %>%
                ...
                addControl(
                  "Color mapping failed - using single color",
                  position = "topright"
                )
            }
          )
      ...
      return(list(
        combined_data = combined_data,
        map = map,
        full_data_range = if (exists("full_data_range")) {
          full_data_range
        } else {
          NULL
        }
