calculate_geometric_mean <- function(combined_data) {
  ...
  if(length(score_cols) > 0) {
    ...
    # Filter out rows where Geo_mean is NaN (meaning ALL inputs were NA)
    combined_data <- combined_data[!is.nan(combined_data$Geo_mean) & !is.na(combined_data$Geo_mean), ]
