...
params:
  ...
  submodel_weights_normalized: NULL
...
```{r}
  ...
  submodel_config <- data.frame(
    ...
    Normalized_Weight = sapply(names(params$submodels_used), function(x) {
      weight <- params$submodel_weights_normalized[[x]] %||% 0
      paste0(round(weight * 100, 1), "%")
    }),
  ...
  kable(submodel_config,
        col.names = c("Submodel", "Status", "Original Weight", "Normalized Weight"),
        format = "html",
...
```{r}
if (!is.null(params$full_map_cropped_normalized)) {
  params$full_map_cropped_normalized
} else {
