...
The Natural Resources submodel includes the following components:
```{r}
if (!is.null(params$natural_resources_components) && length(params$natural_resources_components) > 0) {
  ...
  if (!is.null(params$weight_natural_resources)) {
    normalized_weight_pct <- round((params$submodel_weights$natural_resources %||% 0) * 100, 1)
    cat("**Natural Resources Weight:** ", params$weight_natural_resources, 
        " (", params$weight_natural_resources * 100, "%), normalized weight ", normalized_weight_pct, "%\n\n")
  }
} else {
...
The fisheries submodel includes the following components: 
```{r}
if (!is.null(params$fisheries_components) && length(params$fisheries_components) > 0) {
  ...
  # Add weight information
  if (!is.null(params$weight_fisheries)) {
    normalized_weight_pct <- round((params$submodel_weights$fisheries %||% 0) * 100, 1)
    cat("**Fisheries Weight:** ", params$weight_fisheries, 
        " (", params$weight_fisheries * 100, "%), normalized weight ", normalized_weight_pct, "%\n\n")
  }
} else {
...
The Industry & Operations submodel includes the following components:
```{r}
if (!is.null(params$industry_operations_components) && length(params$industry_operations_components) > 0) {
  ...
  # Add weight information
  if (!is.null(params$weight_industry_operations)) {
    normalized_weight_pct <- round((params$submodel_weights$industry_operations %||% 0) * 100, 1)
    cat("**Industry & Operations Weight:** ", params$weight_industry_operations, 
        " (", params$weight_industry_operations * 100, "%), normalized weight ", normalized_weight_pct, "%\n\n")
  }
} else {
...
if (!is.null(params$component_layer_details) && length(params$component_layer_details) > 0) {
  ...
  if (!is.null(params$data_timestamps)) {
    data_timestamps_df <- params$data_timestamps %>%
      ...
      rename(Layer_Name = dataset_name, Last_Updated = formatted_date)
  }
  ...
  if (length(layer_details_list) > 0) {
    ...
    if (!is.null(data_timestamps_df)) {
      all_layer_details <- all_layer_details %>%
        ...
        mutate(Last_Updated = ifelse(is.na(Last_Updated), "Unknown", Last_Updated))
      # display table with timestamps
      kable(all_layer_details,
            col.names = c("Layer Name", "Selected Score", "Component", "Component Calculation Method", "Submodel", "Last Updated"),
            format = "html",
            table.attr = "class='table table-striped config-table'")
    } else {
