...
```{r}
  ...
  submodel_config <- data.frame(
    ...
    Status = "Included",
    Original_Weight = sapply(names(params$submodels_used), function(x) {
      weight <- params$submodel_weights_original[[x]] %||% 0
      round(weight, 2)
    }),
  ...
  kable(submodel_config,
        col.names = c("Submodel", "Status", "Applied Weight"),
        format = "html",
        row.names = FALSE,
...
if (!is.null(params$component_layer_details) && length(params$component_layer_details) > 0) {
  ...
  if (length(layer_details_list) > 0) {
    ...
    # Sort by Submodel first, then by component, then by layer name
    all_layer_details <- all_layer_details %>% 
      arrange(Submodel, Component, Layer_Name)
    ...
    if (!is.null(data_timestamps_df)) {
      ...
      kable(all_layer_details,
            col.names = c("Submodel", "Layer Name", "Selected Score", "Component", "Component Calculation Method", "Data Type", "Last Updated"),
            format = "html",
            table.attr = "class='table table-striped config-table'")
    } else {
...
This map shows the same full combined model data cropped to the boundaries of the selected area of interest. This map is on the same color scale as the full model maintaing the original full model color scale for consistency with the complete dataset. Scores are distributed along a 0 to 1 scale. Values closer to 0 indicate a less suitable grid cell for offshore wind energy and values closer to 1 indicate a cell that is more suitable for offshore wind energy. 

```{r}
if (!is.null(params$full_map_cropped)) {
  params$full_map_cropped
} else {
